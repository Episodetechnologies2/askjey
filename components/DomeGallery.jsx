...
export default function DomeGallery({
  ...
  // Auto-spin animation for both mobile and desktop
  useEffect(() => {
    const spinSpeed = 0.1; // degrees per frame (slower for smooth continuous rotation)

    const step = () => {
      const isFocused = !!focusedElRef.current;
      const isOpening = openingRef.current;
      const isDragging = draggingRef.current;
      const hasInertia = !!inertiaRAF.current;

      // Only spin when no image is focused, not opening, not dragging, and no inertia
      if (!isFocused && !isOpening && !isDragging && !hasInertia) {
      ...
      // Always schedule next frame to keep the loop running
    ...
    (vx, vy) => {
      ...
      const step = () => {
        ...
        if (Math.abs(vX) < stopThreshold && Math.abs(vY) < stopThreshold) {
          inertiaRAF.current = null;
          // Auto-spin will resume automatically since inertiaRAF is now null
          return;
        }
        if (++frames > maxFrames) {
          inertiaRAF.current = null;
  ...
  useGesture(
    ...
    { target: isMobile ? null : mainRef, eventOptions: { passive: false } }
  );
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (originalImg) {
        ...
        img.style.cssText =
          "width: 100%; height: 100%; object-fit: contain; background: #ffffff; padding: 12px; box-sizing: border-box;";
  ...
  const openItemFromElement = (el) => {
    ...
    closeBtn.onmouseenter = () => {
      ...
      closeBtn.style.background = "rgba(30,188,199,0.15)";
      closeBtn.style.borderColor = "rgba(30,188,199,0.5)";
    };
